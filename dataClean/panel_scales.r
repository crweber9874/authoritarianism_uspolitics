...
# Load panel data -- a list of data frames
load("clean/panels.rda")


### Construct authoritarianism score in each panel########
library(haven)
panel_data_2000 <- dat[[1]]
head(panel_data_2000$auth1.2000)
dat1 <- subset(panel_data_2000, white.2000 == 1) %>%
  zap_labels() %>%
...
panel_data_2012 <- dat[[2]]
dat2 <- subset(panel_data_2012, white.2012 == 1) %>%
  zap_labels() %>%
...
panel_data_2016 <- dat[[3]]
dat3 <- subset(panel_data_2016, white.2016 == 1) %>%
  zap_labels() %>%
...
dat1_long <- data.frame(
  id = rep(1:length(tmp_data$id), times = 2),
...
data <- list(dat1_long, dat2_long, dat3_long, dat1, dat2, dat3)
save(data, file = paste0(data_location, "panel.auth.rda"))
