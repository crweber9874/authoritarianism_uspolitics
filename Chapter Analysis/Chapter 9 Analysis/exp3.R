...
ggtheme = theme(
  plot.title=element_text(face="bold",hjust=0,vjust=0,colour="#3C3C3C",size=16),
...
##### load data from csv
wff3<-read.csv("wff3.csv")
demo3<-read.csv("demo3.csv")

##### make sure case ID variable has same name in both dataframes
demo3$RESPONDENT_ID<-demo3$hash_value

##### merge survey + demographics
exp3 <- left_join(wff3, demo3, by="RESPONDENT_ID")

##### retain check-passed cases only
data <- exp3 %>% dplyr::filter(exp3$atnchk=="2") 
...
data$college<-factor(data$college, labels = c("Non-College", "College"))
...
f8 <- plot_model(m1q, vcov.type = "HC3",
  ...
  scale_x_discrete(limits=c("Economics\nOnly", 
                            "Economics +\nAnti-Immigration", 
                            "Economics +\nAnti-Democracy",
                            "Economics +\nAnti-Immigration +\nAnti-Democracy"))+
...
m2q <- lm(rcsup2 ~ female+age+rinc+college+jewish+cath+other+raut*cond*pid3+
           I(raut^2)*cond*pid3, data=wdata)
coeftest(m2q, vcovHC(m2q, type = "HC3"))

# interaction tests
lht(m2q, test="F", c("raut:condtreat1:pid3Republican=0", 
                     "condtreat1:pid3Republican:I(raut^2)=0"), 
    white.adjust = "hc3")
lht(m2q, test="F", c("raut:condtreat2:pid3Republican=0", 
                     "condtreat2:pid3Republican:I(raut^2)=0"), 
    white.adjust = "hc3")
lht(m2q, test="F", c("raut:condtreat3:pid3Republican=0", 
                     "condtreat3:pid3Republican:I(raut^2)=0"), 
    white.adjust = "hc3")

# marginal effects 
me12<-ggeffects::ggpredict(m2q, vcov.type = "HC3",
                         terms = c("cond", "raut [1, 0]", 
                                   "pid3 [Democrat, Republican]"))
me12
me22<-ggeffects::hypothesis_test(me12)
me22[c(1, 55, 93, 115, 30, 76, 106 , 120),]

# plot, figure 2
# refit with new labels for party
wdata$pid3r<-factor(wdata$pid3, labels = c("Democratic Respondents", 
                                        "Independent", 
...
m2q <- lm_robust(rcsup2 ~ female+age+rinc+college+jewish+cath+other+
                   raut*cond*pid3+I(raut^2)*cond*pid3, data=wdata, 
                 se_type = "HC3")
